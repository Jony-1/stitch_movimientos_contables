...
async function initMovementsPage() {
  ...
  function getVisibleRows(rows) {
    ...
    return rows.filter((row) => String(row.type || "").toLowerCase() === currentFilter);
  }
  ...
  function renderRows(rows) {
    ...
    tbody.innerHTML = visibleRows.map((m) => `
      <tr>
        <td class="px-4 py-4 text-sm text-gray-600">${formatDate(m.date)}</td>
        <td class="px-4 py-4 text-sm">${getAmountLabel(m.type)}</td>
        <td class="px-4 py-4 text-sm text-gray-700">${m.category || ""}</td>
        <td class="px-4 py-4 text-sm text-gray-700">${m.description || ""}</td>
    ...
    if (status) status.textContent = `${visibleRows.length} movimiento(s) cargado(s).`;
