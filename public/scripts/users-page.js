...
async function renderUsers(container, currentUser) {
  ...
  container.innerHTML = `<div class="rounded-2xl border border-slate-200 bg-white/95 p-6 text-sm text-gray-500 shadow-[0_18px_40px_rgba(15,23,42,0.08)] backdrop-blur-xl dark:text-gray-400">Cargando usuarios...</div>`;

  try {
    const rows = await apiJson("/api/users");
    container.innerHTML = rows.length
      ? `<div class="rounded-2xl border border-slate-200 bg-white/95 shadow-[0_18px_40px_rgba(15,23,42,0.08)] backdrop-blur-xl overflow-hidden">
        <table class="w-full">
          <thead class="bg-gray-50 dark:bg-gray-900">
            <tr>
              <th class="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wider text-gray-600 dark:text-gray-300">Nombre</th>
              <th class="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wider text-gray-600 dark:text-gray-300">Correo</th>
              <th class="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wider text-gray-600 dark:text-gray-300">Rol</th>
          ...
          <tbody class="divide-y divide-gray-200 dark:divide-gray-800">
            ${rows.map((u) => `
                  <tr class="hover:bg-gray-50 dark:hover:bg-gray-800/50 transition-colors">
                    <td data-label="Nombre" class="px-4 py-3 text-sm font-medium text-gray-900 dark:text-white">${u.name || ""}</td>
                    <td data-label="Correo" class="px-4 py-3 text-sm text-gray-500 dark:text-gray-400">${u.email || ""}</td>
                    <td data-label="Rol" class="px-4 py-3 text-sm text-gray-600 dark:text-gray-300">${u.role || ""}</td>
                    <td data-label="Estado" class="px-4 py-3 text-sm">
                      <span class="inline-flex items-center gap-2 rounded-full px-2.5 py-0.5 text-xs font-semibold ${u.active ? "bg-green-100 text-green-800 dark:bg-green-900/40 dark:text-green-300" : "bg-gray-100 text-gray-700 dark:bg-gray-800 dark:text-gray-300"}">
                        <span class="size-2 rounded-full ${u.active ? "bg-green-500" : "bg-gray-400"}"></span>
      ...
      : `<div class="rounded-2xl border border-slate-200 bg-white/95 shadow-[0_18px_40px_rgba(15,23,42,0.08)] backdrop-blur-xl p-6 text-sm text-gray-500 dark:text-gray-400">No hay usuarios.</div>`;

    if (status) status.textContent = rows.length ? `${rows.length} usuario(s) cargado(s).` : "No hay usuarios registrados.";
    ...
    container.innerHTML = `<div class="rounded-2xl border border-slate-200 bg-white/95 p-6 text-sm text-red-600 shadow-[0_18px_40px_rgba(15,23,42,0.08)] backdrop-blur-xl">${error.message || "No se pudieron cargar los usuarios."}</div>`;
